...
$sources += @('RecordedCast','ReplayAttempt','ReplayBuffer','ReplayEvidence','ReplayValidation','EvidenceActorIdentity','PullValidationRunner','PositionCheck','PositionCheckSession' | ForEach-Object { "$root/Shikari/Services/Replay/$_.cs" })
$sources += "$PSScriptRoot/PositionCheckSessionTests.cs"
...
[Shikari.Tests.PositionCheckSessionTests]::Run()
